import { GuildiumEnhancementSettings } from "../../../../../common/guildium-settings";
...
const React = window.Guildium.getApiProperty("react"),
    { default: SearchBarInput } = window.Guildium.getApiProperty("guilded/components/SearchBarV2"),
    { default: NullState } = window.Guildium.getApiProperty("guilded/components/NullState"),
    { default: GuildedSelect } = window.Guildium.getApiProperty("guilded/components/GuildedSelect");
...
export class EnhancementGrid<T extends AnyEnhancement, C extends RGEnhancementConfig<T>, S extends GuildiumEnhancementSettings = GuildiumEnhancementSettings> extends React.Component<
...
> {
    ...
    private renderItems(nullStateType: string, nullStateTitle: string, nullStateSubtitle: string, items: T[]) {
        ...
        return (
            items.length
                ? <div className="GuildiumEnhancements-grid UserProfileGamesTab-grid">
    ...
    render() {
        ...
        return (
            <div className="GuildiumEnhancements-container UserProfileGamesTab-container ContentLoader-container">
                <div className="GuildiumEnhancements-topbar">
                    <SearchBarInput className="GuildiumEnhancements-search" searchTerm={searchInput} placeholder={_searchPlaceholder} onChange={_onSearchBinded} />
                    <span />
                    <GuildedSelect className="GuildiumEnhancements-sort" defaultValue={sortingOptions[0]} options={sortingOptions} grow={0} onChange={option => this.setState({ sort: option.value })} />
                </div>
                {searchInput
