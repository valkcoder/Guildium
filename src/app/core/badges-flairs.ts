...
export function createFlairFromBadge(badge: UserBadge): UserFlair {
    const displayInfo = window.Guildium.getApiProperty("guilded/users/flairs/displayInfo").default,
        tooltipInfo = window.Guildium.getApiProperty("guilded/users/flairs/tooltipInfo").default;
...
function generateBadgeGetter<T>(
    ...
    return function get() {
        ...
        const guildiumBadges: T[] = [];

        for (let badgeName in badgeTable) {
            const badge = badgeTable[badgeName];

            if (~membersTable[badgeName].indexOf(this.userInfo.id)) guildiumBadges.push(badge);
        }

        return guildiumBadges.length ? (global || []).concat(guildiumBadges) : global;
    };
}
...
/**
 * Badges/Flairs that are visible on Guildium client.
 */
export const types = {
    dev: {
