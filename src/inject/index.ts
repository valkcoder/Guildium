...
(async function () {
    // Directory to install Guildium configuration to
    ...
    new Promise<boolean>((resolve, reject) => {
        if (specialTasks.includes(taskArg)) {
            access(platform.appDir, constants.F_OK, (err) => {
                if (err && taskArg === "uninject") reject("Called Uninject, but Guildium is not injected.");
                if (!err && taskArg === "inject") reject("Called Inject, but Guildium is already injected.");
        ...
        if (requireElevate) {
            ...
            sudoExec(process.argv.map(x => JSON.stringify(x)).join(" "), {name: "Guildium"}, (err, stdout, stderr) => {
