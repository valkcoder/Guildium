...
import GuildiumWindow from "./guildium-window";
...
if (platform === "linux" && getuid() === 0) {
    console.warn(
        ...
        "Seems this application was ran as root, it has been closed by Guildium to prevent issues, run as a regular user instead!"
    );
    exit(1);
}
...
ipcMain.on("guildium-preload", event => {
    event.returnValue = event.sender.guildedPreload;
});
ipcMain.handle("guildium-enhancement-dialog", async (_, type) => {
...
app.whenReady().then(() => {
    ...
    new Promise(resolve => {
        ...
        .then(() => {
            ...
            try {
                patchCSP();
                /* ipcMain.handle("guildium-repatch-csp", (...args) => {
